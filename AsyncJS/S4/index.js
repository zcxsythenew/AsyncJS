...
function enable_buttons() {
    ...
    if (all_get) {
        ...
        if (auto) {
            timeoutProcess = setTimeout(function () {
                $("#control-ring li")[indexes[current_index + 1]].click();
            }, 500);
        }
	}
}
...
function xmlhttp_onreadystatechange() {
    ...
    if (xmlhttp.readyState === 4 && xmlhttp.status === 200) {
        get_numbers[indexes[current_index]] = xmlhttp.responseText;
        $("#control-ring li")[indexes[current_index]].childNodes[1].innerText = get_numbers[indexes[current_index]];
        enable_buttons();
    }
}
...
$(document).ready(function () {
    ...
    $(".icon").click(function () {
        ...
        if (!auto) {
            ...
            for (i = 0; i < 5; i++) {
                str = str + str_index[indexes[i]];
            }
            $(".tip").text(str);
            $("#control-ring li")[indexes[0]].click();
        }
	});
